...
13、什么是闭包？
闭包就是一个函数里面返回了另一个函数，并且有外部变量接收，形成了一个不销毁的作用域，这个私有作用域，有保护的机制，不让作用域里面的私有变量受到外部的干扰。

14、在函数编程当中有一个很重要的概念，就是函数组合，实际上就是把处理数据的函数像管道连接起来，然后让数据穿过管道，得到最终的结果,例如：
```
 const add1=>(x)=>x+1;
 const mul3=>(x)=>x*3;
 const div2=>(x)=>x/2;
 // 先执行最里层的add1(0)=>得到返回结果1，再执行外层的///add1(1)=>得到返回结果6，再执行div2(6),得到最终的结果3；
...
15、柯理化函数：就是一个函数执行，并且被外部的变量接收，形成不销毁的作用域，预留的参数只传递进去，先不执行，等用到的时候再用，这种思想就是柯理化函数
```
 const add1=>(x)=>x+1;
 const mul3=>(x)=>x*3;
 const div2=>(x)=>x/2;
